  ...
  for (let i = 0; i < arrSearchHistory.length; i++) {
    ...
    $(liEl).on("click", (event) => {
      ...
      (async function () {
        ...
        let idx = arrSearchHistory.findIndex(
          (el) => el.city.toUpperCase() === city.toUpperCase()
        );

        // fetch current weather data for saved city
        let apiObj = await fetchApiWeatherData(
          arrSearchHistory[idx].lat,
          arrSearchHistory[idx].lon
        );
        renderCurrentForecast(apiObj);

        // fetch 5 day forecast weather data for saved city
        apiObj = await fetchApiForecastData(
          arrSearchHistory[idx].lat,
          arrSearchHistory[idx].lon
        );
...
function renderCurrentForecast(apiObj) {
  ...
  let spanEl = $("<span>").text(
    " " + moment(dt, "ddd mm YYYY").format("(MM/DD/YYYY)")
  ); // format the date
  let imgEl = $("<img>")
    .attr(
      "src",
      `https://openweathermap.org/img/wn/${apiObj.weather[0].icon}@2x.png`
    )
    .addClass("p-0 pb-3");
  $(spanEl).append(imgEl);
...
function renderFiveDayForecast(apiObj) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    let cardBodyEl = $("<div>").addClass(
      "card-body d-flex flex-column custom-card-body p-1"
    );

    let h5El = $("<h5>").text(
      moment(apiObj.list[i].dt_txt, "YYYY-MM-DD").format("MM/DD/YYYY")
    ); // format timestamp
    $(cardBodyEl).append(h5El);

    let imgEl = $("<img>").attr(
      "src",
      `https://openweathermap.org/img/wn/${apiObj.list[i].weather[0].icon}@2x.png`
    );
    $(cardBodyEl).append(imgEl);

    let pEl = $("<p>").text(
      "Temp: " + Math.ceil(apiObj.list[i].main.temp) + "°F"
    );
...
var formSubmitHandler = function (arrSearchHistory, event) {
  ...
  let findIndex = arrSearchHistory.findIndex(
    (el) => el.city.toUpperCase() === cityInput.toUpperCase()
  );
